...
app.prepare().then(() => {
  ...
  if (program.args.length > 0) {
    chokidar
      ...
      .on(program.event, async (filePathContext: any, eventContext = defaultWatchEvent) => {
  ...
  reloadRoute.all('/', (req: { body: { message: any; color: any } }, res: { end: (arg0: string) => void }) => {
  ...
  expressApp.all('*', (req: { url: any }, res: any) => handle(req, res, parse(req.url, true)))
