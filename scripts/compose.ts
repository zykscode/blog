...
const getAuthors = (): string[] => {
  const authorPath: string = path.join(root, 'data', 'authors');
  const authorList: string[] = readdirSync(authorPath).map((filename: string) => path.parse(filename).name);
  return authorList;
}


const getLayouts = (): string[] => {
  const layoutPath: string = path.join(root, 'layouts');
  const layoutList: string[] = fs
    .readdirSync(layoutPath)
    .map((filename: string) => path.parse(filename).name)
    .filter((file: string) => file.toLowerCase().includes('post'));
  return layoutList;
}
